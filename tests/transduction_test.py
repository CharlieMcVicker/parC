...
def test_contingent_lexical_submapping():
    ...
    try:
        ...
        parses_1 = fsm_strings(grammar.parse(fsa_1, cascade_domain))
        ...
        parses_2 = fsm_strings(grammar.parse(fsa_2, cascade_domain))
        ...
        parses_3 = fsm_strings(grammar.parse(fsa_3, cascade_domain))
        ...
        parses_4 = fsm_strings(grammar.parse(fsa_4, cascade_domain))
...
def test_build_inflect_graph_for_root_regex():
    ...
    surface_forms = inflect("habl", feature_values, paradigm_name)
...
def test_build_inflect_graph_for_root_regex_with_lexical_features():
    ...
    try:
        # Compile grammar using root regex AND custom lexical features for class1
        grammar = compile_paradigm_grammar(
            "verb_regex_class_test",
            infer_lexical_features=False,
            lexical_features=((class_name, class1),),
        )
...
def test_build_inflect_graph_for_root_regex_lexical_inference():
    ...
    try:
        # Compile paradigm grammar with inference enabled
        grammar = compile_paradigm_grammar(
            "verb_inference_test",
...
def test_build_inflect_graph_for_root_regex_verb_a_stem():
    ...
    surface_forms = inflect("habl", feature_values, "verb_a_stem")
    assert "habl-as" in surface_forms
